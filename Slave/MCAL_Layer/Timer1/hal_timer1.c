...
Std_ReturnType Timer1_Init(const timer1_t *_timer){
    ...
    if(NULL == _timer){
        ...
        #endif
        TMR1_Interrupt_Handler=_timer->TMR1_InterruptHandler;
        #endif  
...
static inline void Timer1_Mode_Select(const timer1_t *_timer){
    if(TIMER1_TIMER_MODE == _timer->timer1_mode){
        ...
        if(TIMER1_ASNYC_COUNTER_MODE == _timer->timer1_counter_mode){
             TIMER1_ASNYC_COUNTER_MODE_ENABLE();
        }
        else if(TIMER1_SYNC_COUNTER_MODE == _timer->timer1_counter_mode){
            TIMER1_SNYC_COUNTER_MODE_ENABLE();
        }
